...
def knn(embedding, matrix_embeddings, labels, k):
    ...
    dist_vector = distance(embedding, matrix_embeddings)
...
def euclidean_distance(xi, X):
    dif_matrix = np.subtract(np.transpose(X), xi)
    return np.linalg.norm(dif_matrix, axis=1)


def manhattan_distance(xi, X):
    dif_matrix = np.abs(np.subtract(np.transpose(X), xi))
