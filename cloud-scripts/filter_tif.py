# This code is create for processing planet image in .tif format.
...
#
# 2. Each function read the .tif image on the disk and write the output image to the disk.
#   The code assumes a planet scene/AOI has four .tif images: Blue(b1), Green(b2), Red(b3), and Near Infrared(b4). The code also includes
#   comments for processing .tif image which has multibands.

# 3. This code provides "horizontal" functions, meaning the functions can be used for more purposes
...
def maxmin_img(m, out_name, *args):
    # def maxmin_img(m,in_name,out_name):  #this is for processing multibands tif image
    """
    ...
    in_name     - input .tif format (multibands)image's full path.
    out_name    - The out put image name. e.g., D:\output.rst
    *args       - input .tif format images' full path.
    """
    try:
        ...
        #########This block is for multibands tif#########
        # in_ds = gdal.Open(in_name)
        # in_ds_GeoTransform = in_ds.GetGeoTransform()
        # in_ds_Projection = in_ds.GetProjection()

        # cols = in_ds.GetRasterBand(1).XSize
        # rows = in_ds.GetRasterBand(1).YSize

        # #2. Create a list of opened bands for later iterations
        # bands = []
        # for band_num in range(1,5):
        #     band = in_ds.GetRasterBand(band_num)
        #     bands.append(band)
        ##################################################
        ...
        out_ds = rst_driver.Create(
            out_name, cols, rows, 1, 3
        )  # here 1 stands for create 1 band, 3 stands for integer data type.
        ...
        for col_start in range(0, cols, block_size):
            ...
            for row_start in range(0, rows, block_size):
                ...
                for band in bands:
                    current_block = band.ReadAsArray(
                        col_start, row_start, col_read, row_read
                    )
                    bands_blocks.append(current_block)

                block_stack = np.dstack(
                    bands_blocks
                )  # stack the bands along axis2(layers of a pixel)
                if m == "max":
...
def reclass(cd, in_name, out_name, thre_val):
    ...
    try:
        ...
        # 2.Create a output file.
        rst_driver = gdal.GetDriverByName("GTiff")
        out_ds = rst_driver.Create(
            out_name, cols, rows, 1, 3
        )  # here 1 means 1 band in the image,3 means integer datatype
        out_band = out_ds.GetRasterBand(1)
        ...
        for col_start in range(0, cols, block_size):
            ...
            for row_start in range(0, rows, block_size):
                ...
                current_block = in_band.ReadAsArray(
                    col_start, row_start, col_read, row_read
                )

                if cd == ">":
                    reclass_arry = np.where(current_block > thre_val, 1, 0).astype(
                        np.int16
                    )
                elif cd == "<":
                    reclass_arry = np.where(current_block < thre_val, 1, 0).astype(
...
def filter_window(in_name, out_name, filter_size, cd):
    ...
    try:
        ...
        out_ds = rst_driver.Create(
            out_name, cols, rows, 1, 3
        )  # here 1 means 1 band in the image,3 stands for integer datatype
        ...
        if cd == "max":
            ...
            while current_row <= rows - 1:
                if current_row - 3 < 0:
                    current_block = in_band.ReadAsArray(
                        0, 0, cols, current_row + 4
                    )  # col_start, row_start, col read, row read
                    filtered_block = ndimage.maximum_filter(current_block, filter_size)
                    out_band.WriteArray(
                        filtered_block[-4:-3], 0, current_row
                    )  # data, col_start, rows_start

                elif current_row - 3 >= 0 and current_row + 3 <= rows - 1:
                    current_block = in_band.ReadAsArray(0, current_row - 3, cols, 7)
                    filtered_block = ndimage.maximum_filter(current_block, filter_size)
                    out_band.WriteArray(
                        filtered_block[3:4], 0, current_row
                    )  # data, col_start, rows_start

                if current_row + 3 > rows - 1:
                    current_block = in_band.ReadAsArray(
                        0, current_row - 3, cols, rows - 1 - current_row + 4
                    )
                    filtered_block = ndimage.maximum_filter(current_block, filter_size)
                    out_band.WriteArray(
                        filtered_block[3:4], 0, current_row
...
in_path = r"D:\Extracting_Clouds\cloud_shadow_test_tif\\"  # input image path
...
# "20180228_095050_102f_3B_AnalyticMS_SR_b1.tif"
# "20180228_095050_102f_3B_AnalyticMS_SR_b2.tif"
# "20180228_095050_102f_3B_AnalyticMS_SR_b3.tif"
# "20180228_095050_102f_3B_AnalyticMS_SR_b4.tif"


os.makedirs(r"D:\Extracting_Clouds\0726\img\\")  # make an output folder
out_path = r"D:\Extracting_Clouds\0726\img\\"

b1 = in_path + in_name + "b1.tif"
b2 = in_path + in_name + "b2.tif"
b3 = in_path + in_name + "b3.tif"
b4 = in_path + in_name + "b4.tif"


# 2. Calculate the max and min image, and output to disk
maxmin_img("max", out_path + "max_img.tif", b1, b2, b3, b4)
maxmin_img("min", out_path + "min_img.tif", b1, b2, b3, b4)

filter_window(out_path + "max_img.tif", out_path + "max7x7.tif", 7, "max")
filter_window(out_path + "min_img.tif", out_path + "min7x7.tif", 7, "min")

# 3. Calculate Shadow and Water, then to disk
reclass(
    "<", out_path + "max7x7.tif", out_path + "Shadow_and_Water.tif", 2000
)  # specify threshold for shadow and water

# 4. Cloud, and to disk
reclass(
    ">", out_path + "min7x7.tif", out_path + "Cloud.tif", 2000
)  # specify threshold for cloud

# 5. Land, and to disk
reclass(">", b4, out_path + "Land.tif", 1000)  # specify threshold for land

# 6. Shadow, and to disk
masks(out_path + "Shadow_and_Water.tif", out_path + "Land.tif", out_path + "Shadow.tif")

# 7. put shadow and cloud into one image, and to disk

cloud_shadow(
    out_path + "Cloud.tif", out_path + "Shadow.tif", out_path + "Cloud_Shadow.tif"
)
