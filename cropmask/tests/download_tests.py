...
def setup_order(configs):
    ...
    try:
        ...
        scene_list = du.get_scene_list(collection='LANDSAT_TM_C1',
                         bbox=bbox,
...
def test_azure_download(setup_order, configs):
    ...
    setup_order.download_all_complete_azure(configs['storage']['container'],configs["storage"]["storage_name"], configs["storage"]["storage_key"] )

    url = setup_order.urls_completed[-1]
    blob_name = url.split('/')[-1].split('.')[0]

    assert blob.exists(container = configs["storage"]["storage_name"], blob_name = blob_name)
