...
def meiou_loss(y_true, y_pred):
    """Implements GIoU loss.
    y_true and y_pred are typically: [N, 4], but could be any shape.
    """

    # In the format [N, (dy, dx, log(dh), log(dw))]
    y_true = y_true.astype(np.float32)
    y_pred = y_pred.astype(np.float32)
    H_image, W_image = config.IMAGE_SHAPE[:2]

    # TODO :  Verify the denormalization is correct
    # Convert y_pred to standards coordonates y1, x1, y2, x2
    w_p = np.exp(y_pred[:, 2]) * W_image
    h_p = np.exp(y_pred[:, 3]) * H_image
    y1_p = (y_pred[:, 1] * H_image) - w_p * 0.5
    x1_p = (y_pred[:, 0] * W_image) - h_p * 0.5
    y2_p = y1_p + w
    x2_p = x1_p + h
    # Compute y_pred Area
    Ap = w_p * h_p

    # Convert y_true to standards coordonates y1, x1, y2, x2
    w_g = np.exp(y_true[:, 2]) * W_image
    h_g = np.exp(y_true[:, 3]) * H_image
    y1_g = (y_true[:, 1] * H_image) - w_g * 0.5
    x1_g = (y_true[:, 0] * W_image) - h_g * 0.5
    y2_g = y1_g + w
    x2_g = x1_g + h
    # Compute y_true Area
    Ag = w_g * h_g
    ...
    intersection = tf.maximum(x2_i - x1_i, 0) * tf.maximum(y2_i - y1_i, 0)

    # Compute Union
    union = Ap + Ag - intersection
    iou = intersection / union

    # Small enclosing boxes coordonate
    x1_c = tf.minimum(x1_p, x1_g)
    y1_c = tf.minimum(y1_p, y1_g)
    x2_c = tf.maximum(x2_p, x2_g)
    y2_c = tf.maximum(y2_p, y2_g)
    enclosing_box = (xc2 - xc1) * (yc2 - yc1)
    # Compute enclosing box Height and Width
    h_c = K.abs(x2_c - x1_c)
    w_c = K.abs(y2_c - y1_c)

    # MEIOU loss with Mahnattan distance
    loss_meiou = 1 - iou + ((w_p - w_g)/w_c)**2 + ((h_p - h_g)/h_c)**2 + K.abs(y1_p - y1_g)/w_c + K.abs(x1_p - x1_p)/h_c
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    loss = K.switch(tf.size(input=target_bbox) > 0, smooth_l1_loss(y_true=target_bbox, y_pred=pred_bbox), tf.constant(0.0))
    loss = K.mean(loss)
    return loss
