...
class WireframeConfig(Config):
    ...
    NUM_CLASSES = 1 + 4  # Background + Icons
...
class WireframeDataset(utils.Dataset):
    ...
    def load_wireframe(self, dataset_dir, subset):
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = WireframeDataset()
    dataset_train.load_wireframe(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = WireframeDataset()
    dataset_val.load_wireframe(args.dataset, "val")
    dataset_val.prepare()
